...
export default class DomainSelect extends React.Component {
    static propTypes = {
        ...
        filterDataOption: PropTypes.any
    };
    ...
    loadData = (props) => {
        const {dictTypeCode, parentCode, filterDataOption} = props;
        let params = {
            DICT_TYPE_CODE: dictTypeCode || "",
            PARENT_CODE: parentCode || ""
        };
        Ajax.apiPost("/common/pc/systemmanager/systemsetting/dict/data/get-dict-child-list", params)
            .then((d) => {
                let data = d.data || [];
                if (filterDataOption) {
                    data = data.filter((value, index, array) => filterDataOption(value, index, array));
                }
                this.setState({
                    data: data
                });
