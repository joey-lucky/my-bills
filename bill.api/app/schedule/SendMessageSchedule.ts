...
export default class SendMessageSchedule extends Subscription {
    ...
    async subscribe() {
        ...
        try {
            const entityList: BdSendMessage[] = await database.find(BdSendMessage, {
                where: {
                    sendStatus: "0",
                },
            });
    ...
    async sendMessage(entity: BdSendMessage) {
        ...
        try {
            const tokenEntity: BcToken = await database.findOne(BcToken, tokenId);
            const user = await database.findOne(BcUser, userId);
            const {bussWx} = user;
