 ...
 */
@createForm()
export default class IncomeBillAdd extends React.Component {
    _defBill = {
        date_time: new Date(),
    ...
    componentDidMount() {
        billApi.incomeBillAdd.getCardList().then(d => {
            let data = d.data || [];
            this.setState({
                cardData: this.parseDataToPickerData(data)
            });
        });
        billApi.incomeBillAdd.getBillTypeList().then(d => {
            let data = d.data || [];
            data.forEach(item => {
                item.value = item.id;
        ...
        this.props.form.validateFields((error, values) => {
            if (error) {
                ...
                if (this.state.isUpdate) {
                    ...
                    billApi.incomeBillAdd.updateBill(params).then(() => this.props.history.goBack());
                } else {
                    let bill = {...this.state.bill, ...values};
                    ...
                    billApi.incomeBillAdd.createBill(params).then(() => this.props.history.goBack());
                }
            }
        });
    };
